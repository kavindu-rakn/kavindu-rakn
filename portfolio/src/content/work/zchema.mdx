...
liveUrl: https://zchema-kvn.vercel.app/
githubUrl: https://github.com/kavindu-rakn/Zchema
techStack:
  - Next.js 16
  - React 19
  - TypeScript
...
figures:
  ...
  - spec: The migration history, showing a completed migration alongside its version and the rollback that is still available.
  - spec: The schema tab on a phone, showing that inheritance is still legible on a narrow screen.
    viewport: mobile
  - spec: The impact dialog on a phone, with the destructive row expanded.
    viewport: mobile
  - spec: The category tree on a phone, deep enough to show several levels of nesting.
    viewport: mobile
draft: false
---
...
Select a category to see its resolved schema — inherited fields first, in the
order the resolver composes them, then whatever that node declares itself. Type
a field name to watch it compose downward, and to see what the change would
...
thing holding them in step is an identical comment block and the discipline to
update both. That is the weakest part of the design. I would add a test that runs both against the
